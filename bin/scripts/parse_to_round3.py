...
dirt_csv_dirt = dirt_base + "round_2_inconsistent/"
out_put_dirt = dirt_base + "round_3/"
temp_work_dirt = dirt_base + "round_3/temp/"
...
print(f"Raw clustering files for Round 3 have been generated at {dirt_base}")

# Load previous round mappings
with open(dirt_csv_dirt + 'cluster_members_round_2_inconsistent.json') as input_1st:
    cluster_result_readin_1st = json.load(input_1st)

# Define a function to map Round 3 clusters back to Round 1 based on previous rounds
def generate_final_mapping_to_barcodes(current_round_result, previous_rounds, output_dir, round_name):
    ...
    for cluster_id, members in current_round_result.items():
        ...
        for member in members:
            if member in previous_rounds["round_2"]:
                barcodes.extend(previous_rounds["round_2"][member])
...
# Create the final mapping based on Round 3 clusters
previous_rounds = {"round_2": cluster_result_readin_1st}
generate_final_mapping_to_barcodes(cluster_result, previous_rounds, out_put_dirt, "round_3")
...
def main():
    cluster_file = dirt_base + 'round_3/cluster_members_round_3.raw.original.tsv'
    merged_output = dirt_base + 'round_3/merged_output_round_3.tsv'
    evaluation_output = dirt_base + 'round_3/cluster_evaluation_round_3.tsv'
    filtered_tsv = dirt_base + 'round_3/cluster_members_round_3.tsv'
    filtered_samples_file = dirt_base + 'round_3/contaminated_samples.tsv'
...
def compare_annotations_and_save_results(dirt_base, round_2_file, round_3_file, json_mapping, merged_output_file, original_json_file, final_json_file, log_file):
    # Load evaluation data
    round_2_eval = pd.read_csv(dirt_base + round_2_file, sep='\t')
    round_3_eval = pd.read_csv(dirt_base + round_3_file, sep='\t')
    merged_output = pd.read_csv(dirt_base + merged_output_file, sep='\t')

    # Replace NaN with 'N/A'
    round_2_eval['evaluation'] = round_2_eval['evaluation'].fillna('N/A')
    ...
    # Mapping from round 2
    round_2_mapping = round_2_eval.set_index('cluster_id')['evaluation'].to_dict()
    ...
    for cluster_id, members in cluster_mapping.items():
        round_3_evaluation = round_3_eval.loc[round_3_eval['cluster_id'] == int(cluster_id), 'evaluation'].values[0]
        round_2_evaluation_set = {round_2_mapping.get(int(member), 'N/A') for member in members}

        # Check taxa data
        taxa_data = merged_output[merged_output['cluster_id'] == int(cluster_id)]['rank.1_taxa'].dropna().unique()
        if round_3_evaluation == 'N/A' and all(taxa == 'N/A' or taxa in round_2_evaluation_set for taxa in taxa_data):
            continue

        # Record inconsistent annotations
        for member in members:
            round_2_evaluation = round_2_mapping.get(int(member), 'N/A')
            if round_2_evaluation != 'N/A' and (round_3_evaluation == 'N/A' or round_2_evaluation != round_3_evaluation):
                print(f'Round 3 Cluster {cluster_id}: {round_3_evaluation}, Round 2 Cluster {member}: {round_2_evaluation}')
                clusters_to_delete.append(str(cluster_id))
                with open(dirt_base + log_file, 'a') as log:
                    log.write(f'Round 3 Cluster {cluster_id}: {round_3_evaluation}, Round 2 Cluster {member}: {round_2_evaluation}\n')
...
def main():
    round_2_file = 'round_2_inconsistent/cluster_evaluation_round_2_inconsistent.tsv'
    round_3_file = 'round_3/cluster_evaluation_round_3.tsv'
    json_mapping = 'round_3/cluster_members_2nd.raw.json'
    merged_output_file = 'round_3/merged_output_round_3.tsv'
    original_json_file = 'round_3/cluster_members_round_3.raw.original.json'
    final_json_file = 'round_3/cluster_members_round_3.de_overfit.json'
    log_file = 'round_3/overfitted.log'
...
def main():
    json_file = dirt_base + 'round_3/cluster_members_round_3.de_overfit.json'
    tsv_file = dirt_base + 'round_3/merged_output_round_3.tsv'
    output_file = dirt_base + 'round_3/merged_output_round_3-de_overfit.tsv'
...
def main():
    merged_output_file = dirt_base + 'round_3/merged_output_round_3-de_overfit.tsv'
    evaluation_output_file = dirt_base + 'round_3/cluster_evaluation_round_3.tsv'

    filtered_tsv = dirt_base + 'round_3/cluster_members_round_3.tsv'
    filtered_json = dirt_base + 'round_3/cluster_members_round_3.final.json'
    filtered_samples_file = dirt_base + 'round_3/contaminated_samples.tsv'
...
final_json_file = dirt_base + 'round_3/cluster_members_round_3.final.json'
