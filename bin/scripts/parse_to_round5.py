...
dirt_csv_dirt = dirt_base + "round_4/"
out_put_dirt = dirt_base + "round_5/"
temp_work_dirt = dirt_base + "round_5/temp/"
...
print(f"Raw clustering files for Round 5 have been generated at {dirt_base}")
...
with open(dirt_base + "round_4/cluster_members_round_4.final.json") as input_3rd:
    cluster_result_readin_3rd = json.load(input_3rd)


# Define a function to map Round 5 clusters back to previous rounds based on mappings
def generate_final_mapping_to_barcodes(current_round_result, previous_rounds, output_dir, round_name):
    ...
    for cluster_id, members in current_round_result.items():
        ...
        for member in members:
            if member in previous_rounds["round_4"]:
                barcodes.extend(previous_rounds["round_4"][member])
...
# Create the final mapping based on Round 5 clusters
previous_rounds = {
    "round_2": cluster_result_readin_1st,
    "round_3": cluster_result_readin_2nd,
    "round_4": cluster_result_readin_3rd  # Ensure "round_4" is correctly added here
}
generate_final_mapping_to_barcodes(cluster_result, previous_rounds, out_put_dirt, "round_5")
...
def main():
    cluster_file = dirt_base + 'round_5/cluster_members_round_5.raw.original.tsv'
    merged_output = dirt_base + 'round_5/merged_output_round_5.tsv'
    evaluation_output = dirt_base + 'round_5/cluster_evaluation_round_5.tsv'
    filtered_tsv = dirt_base + 'round_5/cluster_members_round_5.tsv'
    filtered_samples_file = dirt_base + 'round_5/contaminated_samples.tsv'
...
def compare_annotations_and_save_results(dirt_base, round_4_file, round_5_file, json_mapping, merged_output_file, original_json_file, final_json_file, log_file):
    # Load evaluation data
    round_4_eval = pd.read_csv(dirt_base + round_4_file, sep='\t')
    round_5_eval = pd.read_csv(dirt_base + round_5_file, sep='\t')
    merged_output = pd.read_csv(dirt_base + merged_output_file, sep='\t')

    # Replace NaN with 'N/A'
    round_4_eval['evaluation'] = round_4_eval['evaluation'].fillna('N/A')
    round_5_eval['evaluation'] = round_5_eval['evaluation'].fillna('N/A')
    ...
    # Mapping from round 4
    round_4_mapping = round_4_eval.set_index('cluster_id')['evaluation'].to_dict()
    ...
    for cluster_id, members in cluster_mapping.items():
        round_5_evaluation = round_5_eval.loc[round_5_eval['cluster_id'] == int(cluster_id), 'evaluation'].values[0]
        round_4_evaluation_set = {round_4_mapping.get(int(member), 'N/A') for member in members}

        # Check taxa data
        taxa_data = merged_output[merged_output['cluster_id'] == int(cluster_id)]['rank.1_taxa'].dropna().unique()
        if round_5_evaluation == 'N/A' and all(taxa == 'N/A' or taxa in round_4_evaluation_set for taxa in taxa_data):
            continue

        # Record inconsistent annotations
        for member in members:
            round_4_evaluation = round_4_mapping.get(int(member), 'N/A')
            if round_4_evaluation != 'N/A' and (round_5_evaluation == 'N/A' or round_4_evaluation != round_5_evaluation):
                print(f'Round 5 Cluster {cluster_id}: {round_5_evaluation}, Round 4 Cluster {member}: {round_4_evaluation}')
                clusters_to_delete.append(str(cluster_id))
                with open(dirt_base + log_file, 'a') as log:
                    log.write(f'Round 5 Cluster {cluster_id}: {round_5_evaluation}, Round 4 Cluster {member}: {round_4_evaluation}\n')
...
def main():
    ...
    round_5_file = 'round_5/cluster_evaluation_round_5.tsv'
    json_mapping = 'round_5/cluster_members_4th.raw.json'
    merged_output_file = 'round_5/merged_output_round_5.tsv'
    original_json_file = 'round_5/cluster_members_round_5.raw.original.json'
    final_json_file = 'round_5/cluster_members_round_5.de_overfit.json'
    log_file = 'round_5/overfitted.log'
...
def main():
    ...
    json_file = dirt_base + 'round_5/cluster_members_round_5.de_overfit.json'
    tsv_file = dirt_base + 'round_5/merged_output_round_5.tsv'
    output_file = dirt_base + 'round_5/merged_output_round_5-de_overfit.tsv'
...
def main():
    merged_output_file = dirt_base + 'round_5/merged_output_round_5-de_overfit.tsv'
    evaluation_output_file = dirt_base + 'round_5/cluster_evaluation_round_5.tsv'

    filtered_tsv = dirt_base + 'round_5/cluster_members_round_5.tsv'
    filtered_json = dirt_base + 'round_5/cluster_members_round_5.final.json'
    filtered_samples_file = dirt_base + 'round_5/contaminated_samples.tsv'
...
final_json_file = dirt_base + 'round_5/cluster_members_round_5.final.json'
