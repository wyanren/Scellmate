...
compute_avg_and_median_contig_length() {
  local fasta_dir=$1
  local tmp="$fasta_dir/file_totals.tmp"

  > "$tmp"
  for f in "$fasta_dir"/*.fasta; do
    [ -f "$f" ] || continue
    total_bp=$(bioawk -c fastx '{sum+=length($seq)} END{print sum}' "$f")
    echo "$total_bp" >> "$tmp"
  done

  sort -n "$tmp" -o "$tmp"

  local count=$(wc -l < "$tmp")
  local avg=0
  local median=0

  if (( count > 0 )); then
    avg=$(awk -v n="$count" '{sum+=$1} END{printf "%.0f", sum/n}' "$tmp")
    if (( count % 2 == 1 )); then
      median=$(awk -v mid=$((count/2+1)) 'NR==mid{print $1}' "$tmp")
    else
      median=$(awk -v mid1=$((count/2)) -v mid2=$((count/2+1)) \
        'NR==mid1{a=$1} NR==mid2{printf "%.0f", (a+$1)/2}' "$tmp")
    fi
  fi
...
if cut -f1 "$logfile" | grep -q '^stop_clustering$'; then
  echo "[INFO] Detected previous stop_clustering, exiting."
  exit 0
fi
##########################################################################################
##########            round_1 → assembly               #########
...
if step_done "round_1"; then
  echo "[INFO] round_1 already done, skipping assembly and stats."  
else
  ...
  # exclude those contaminated SAGs in first round QC in <round_1/spades_output_filtered_sample/>
  ...
  # perform MGE masking in <round_1/spades_output_filter/>
  ...
  read avg_len median_len <<< $(compute_avg_and_median_contig_length "$workdir/06_CoSAG_assembly/round_1/spades_output_filtered_sample")
  echo "[INFO] round_1: avg = ${avg_len} bp, median = ${median_len} bp"
  echo "$median_len" > "$workdir/06_CoSAG_assembly/round_1/median.txt"
  log_step "round_1"
fi
...
if step_done "round_2_inconsistent"; then
  echo "[INFO] round_2_inconsistent already done, skipping assembly and stats."
else
  ...
  read avg_len median_len <<< $(compute_avg_and_median_contig_length "$workdir/06_CoSAG_assembly/round_2_inconsistent/spades_output")
  echo "[INFO] round_2_inconsistent: avg = ${avg_len} bp, median = ${median_len} bp"
  echo "$median_len" > "$workdir/06_CoSAG_assembly/round_2_inconsistent/median.txt"
  
  prev_median_file="$workdir/06_CoSAG_assembly/round_1/median.txt"
  prev_median=0
  [[ -f "$prev_median_file" ]] && prev_median=$(<"$prev_median_file")

  prev_median_file="$workdir/06_CoSAG_assembly/round_1/median.txt"
  
  if (( avg_len > 5000000 )); then
    echo "[INFO] avg > 5 Mb → stop further rounds."
    log_step "round_2_inconsistent"
    log_step "stop_clustering"
    exit 0
  fi

  if (( prev_median > 0 )); then
    ratio=$(awk -v c="$median_len" -v p="$prev_median" 'BEGIN{printf "%.3f", c/p}')
    echo "[INFO] median increase ratio = $ratio"
    if (( $(echo "$ratio < 2.0" | bc -l) )); then
      echo "[INFO] median of genome size gain <100% → stop further rounds."
      log_step "round_2_inconsistent"
      log_step "stop_clustering"
      exit 0
    fi
...
if step_done "round_3"; then
  echo "[INFO] round_3 already done, skipping assembly and stats."
else
  ...
  overfit_file="$workdir/06_CoSAG_assembly/round_3/merged_output_round_3-de_overfit.tsv"
  if [[ -f "$overfit_file" ]]; then
    count=$(awk 'NR>1' "$overfit_file" | wc -l)
    if (( count == 0 )); then
      echo "[INFO] round_3: no reliable clusters found in overfit file, stopping clustering."
      log_step "stop_clustering"
      exit 0
    fi
  else
    echo "[WARN] round_3: empty clustering after overfitting check ($overfit_file), stop clustering in round_2_inconsistent."
  fi
  ...
  read avg_len median_len <<< $(compute_avg_and_median_contig_length "$workdir/06_CoSAG_assembly/round_3/spades_output")
  echo "[INFO] round_3: avg = ${avg_len} bp, median = ${median_len} bp"
  echo "$median_len" > "$workdir/06_CoSAG_assembly/round_3/median.txt"
  
  prev_median_file="$workdir/06_CoSAG_assembly/round_2_inconsistent/median.txt"
  prev_median=0
  [[ -f "$prev_median_file" ]] && prev_median=$(<"$prev_median_file")
...
if step_done "round_4"; then
  echo "[INFO] round_4 already done, skipping assembly and stats."
  ...
  overfit_file="$workdir/06_CoSAG_assembly/round_4/merged_output_round_4-de_overfit.tsv"
  if [[ -f "$overfit_file" ]]; then
    count=$(awk 'NR>1' "$overfit_file" | wc -l)
    if (( count == 0 )); then
      echo "[INFO] round_4: no reliable clusters found in overfit file, stopping clustering."
      log_step "stop_clustering"
      exit 0
    fi
  else
    echo "[WARN] round_4: empty clustering after overfitting check ($overfit_file), stop clustering in round_3."
  fi
  ...
  if [[ $num_input4 -gt 0 && $num_input4 -eq $num_fasta4 ]]; then
    ...
    if [[ $num_input4 -gt 0 ]]; then
      per_job4=$(( threads / num_input4 ))
      [[ $per_job4 -lt 1 ]] && per_job4=1
    else
      per_job4=1
    fi

    echo "[INFO] round_4: num_input4=$num_input4, total_threads=$threads, per_job=$per_job4"
  ...
  read avg_len median_len <<< $(compute_avg_and_median_contig_length "$workdir/06_CoSAG_assembly/round_4/spades_output")
  echo "[INFO] round_4: avg = ${avg_len} bp, median = ${median_len} bp"
  echo "$median_len" > "$workdir/06_CoSAG_assembly/round_4/median.txt"
  
  prev_median_file="$workdir/06_CoSAG_assembly/round_3/median.txt"
  prev_median=0
  [[ -f "$prev_median_file" ]] && prev_median=$(<"$prev_median_file")
...
if step_done "round_5"; then
  ...
  overfit_file="$workdir/06_CoSAG_assembly/round_5/merged_output_round_5-de_overfit.tsv"
  if [[ -f "$overfit_file" ]]; then
    count=$(awk 'NR>1' "$overfit_file" | wc -l)
    if (( count == 0 )); then
      echo "[INFO] round_5: no reliable clusters found in overfit file, stopping clustering."
      log_step "stop_clustering"
      exit 0
    fi
  else
    echo "[WARN] round_5: empty clustering after overfitting check ($overfit_file), stop clustering in round_4."
  fi
  ...
  if [[ $num_input5 -gt 0 && $num_input5 -eq $num_fasta5 ]]; then
    ...
    if [[ $num_input5 -gt 0 ]]; then
      per_job5=$(( threads / num_input5 ))
      [[ $per_job5 -lt 1 ]] && per_job5=1
    else
      per_job5=1
    fi

    echo "[INFO] round_5: num_input5=$num_input5, total_threads=$threads, per_job=$per_job5"
  ...
  read avg_len median_len <<< $(compute_avg_and_median_contig_length "$workdir/06_CoSAG_assembly/round_5/spades_output")
  echo "[INFO] round_5: avg = ${avg_len} bp, median = ${median_len} bp"
  echo "$median_len" > "$workdir/06_CoSAG_assembly/round_5/median.txt"
  
  prev_median_file="$workdir/06_CoSAG_assembly/round_4/median.txt"
  prev_median=0
  [[ -f "$prev_median_file" ]] && prev_median=$(<"$prev_median_file")
...
# Fallback: if all rounds completed without triggering any stop condition
if ! cut -f1 "$logfile" | grep -q '^stop_clustering$'; then
  log_step "stop_clustering"
fi

echo -e "\033[0;33m[Scellmate] Iterative clustering and co-assembly to construct CoSAGs in second QC have been completed.\033[0m"
