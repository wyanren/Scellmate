...
threads=20                # parallel jobs for Kraken2 / GNU parallel
##########################################################################################
...
##########################################################################################
KRAKEN2_DB_SRC=($SCELLMATE_GTDB_PATH/kraken2_DB*/)
GTDB_TAXONOMY="$SCELLMATE_GTDB_PATH/taxonomy_r220.processed.tsv"
GTDB_GENOMES_ID="$SCELLMATE_GTDB_PATH/all_genomes.id.txt"

########################  prepare directories  ################################
mkdir -p "$workdir"/{02_kraken2_output,03_reference_db/chromosome}

########################  copy Kraken2 DB to $TMP_FILESYSTEM_PATH (if absent) #############
if [[ ! -d "${TMP_FILESYSTEM_PATH}/kraken2_db" \
      || $(ls "${TMP_FILESYSTEM_PATH}/kraken2_db"/*.k2d 2>/dev/null | wc -l) -ne $(ls "${KRAKEN2_DB_SRC}"/*.k2d 2>/dev/null | wc -l) ]]; then
    echo "[INFO] Copying Kraken2 DB to ${TMP_FILESYSTEM_PATH}..."
    mkdir -p "${TMP_FILESYSTEM_PATH}/kraken2_db"
    cp "${KRAKEN2_DB_SRC}"/*.k2d "${TMP_FILESYSTEM_PATH}/kraken2_db/"
else
    echo "[INFO] ${TMP_FILESYSTEM_PATH}/kraken2_db already exists. Skipping copy."
fi
...
parallel --env workdir --env logfile -j "$threads" '
  sample=$(basename {} _R2_paired.fastq)
  {
    kraken2 --memory-mapping \
      --db "${TMP_FILESYSTEM_PATH}/kraken2_db" \
...
while IFS=$'\t' read -r col1 col2 col3 col4 col5 col6 col7; do
  ...
  matches=$(grep -F "${col6} " "$report_path" || true)
if [[ -n "$matches" ]]; then
  awk -v fname="$col7" 'BEGIN{OFS="\t"} {print $0, fname}' <<< "$matches" \
    >> "${workdir}/03_reference_db/chromosome/temp-species.tsv"
else
  echo "[WARN] no species match for genus [$col6] in $col7" >&2
fi
...
echo -e "[NOTICE] temp-id.gtdb_rep-genus is empty — expected with the reduced GTDB test database; no ambiguous genomes were extracted."
echo -e "\033[1;32m[Scellmate] Subset of prokaryotic genomes in first QC have been completed.\033[0m"
