...
if (skip_unk_filter) {
  ...
  purity_cutoff <- max(unlist(results_dt$corresponding_values))
  ...
  if (length(unk_fail_or_missing) > 0) {
    contaminated_dt <- rbind(
      contaminated_dt,
      data.table(variable = unk_fail_or_missing, sample = "placeholder"),
      fill = TRUE
    )
    contaminated_dt <- unique(contaminated_dt, by = c("variable", "sample"))
  }

  union_variables <- character(0)

  cat(sprintf(
    "[INFO] --skip-unk-filter: %d Unknown SAGs reclassified as pass, %d as fail.\n",
