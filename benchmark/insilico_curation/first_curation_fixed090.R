...
for (pkg in packages) {
  # load quietly, suppressing startup messages
...
option_list <- list(
  make_option(c("-w", "--workdir"), type = "character", default = getwd(),
              ...
              metavar = "DIR"),
  make_option("--skip-unk-filter", action = "store_true", default = FALSE,
              help = "Skip unknown-SAG filtering (mapping ratio, marker-gene count, kraken2 ratio) and go directly to rank.1 purity gating [default: FALSE]")
)

opt_parser <- OptionParser(option_list = option_list)
opt <- parse_args(opt_parser)
workdir <- normalizePath(opt$workdir)
skip_unk_filter <- opt$`skip-unk-filter`

#---------------Check environment variable---------------#
db_dir <- Sys.getenv("SCELLMATE_GTDB_PATH", unset = NA)
if (is.na(db_dir) || db_dir == "") {
  stop("Environment variable SCELLMATE_GTDB_PATH is not set!")
}

#---------------Define input file paths based on workdir---------------#
feature_file <- file.path(workdir, "05_first_QC", "featureCounts", "featureCounts_filtered.txt")
mapped_file  <- file.path(workdir, "05_first_QC", "mapped_stat.txt")
kraken_file  <- file.path(workdir, "05_first_QC", "kraken2_stat.txt")

#---------------Loading tables---------------#
feature <- data.table::fread(feature_file)

mapped_profile <- setNames(
  read.delim(mapped_file, sep = "|", header = TRUE)[, -2],
  c("Cell_ID", "total_mapped", "total_mapped_percentage")
)

kraken2_profile <- read.delim(kraken_file, header = TRUE, sep = '|')
...
if (length(cols_to_remove) > 0) {
  feature_pivoted_GenomeID <- feature_pivoted_GenomeID %>% dplyr::select(-all_of(cols_to_remove))
}
...
#---------------Preprocessing needed by both paths---------------#
...
#---------------Reclassify Unknown SAGs when --skip-unk-filter is set---------------#
if (skip_unk_filter) {
  cat("[INFO] --skip-unk-filter: reclassifying Unknown SAGs using the purity threshold from Section 3.\n")
  purity_cutoff <- max(unlist(results_dt$corresponding_values[[1]]))

  # Check Unknown SAGs' rank.1 values in melt_df against the purity cutoff
  setDT(melt_df)
  unk_in_melt <- melt_df[variable %in% union_variables & Type == "rank.1_taxa"]
  unk_in_melt[, sample := "placeholder"]
  unk_pass_ids <- unk_in_melt[value > purity_cutoff, unique(variable)]
  unk_fail_or_missing <- setdiff(union_variables, unk_pass_ids)

  # Add passing Unknown SAGs to filtered_result
  if (length(unk_pass_ids) > 0) {
    unk_pass_rows <- unk_in_melt[variable %in% unk_pass_ids]
    filtered_result <- rbind(filtered_result, unk_pass_rows, fill = TRUE)
  }

  # Add failing/missing Unknown SAGs to contaminated_dt
  if (length(unk_fail_or_missing) > 0) {
    contaminated_dt <- rbind(contaminated_dt,
                             data.table(variable = unk_fail_or_missing, sample = "placeholder"))
  }

  cat(sprintf("[INFO] --skip-unk-filter: %d Unknown SAGs reclassified as pass, %d as fail.\n",
              length(unk_pass_ids), length(unk_fail_or_missing)))

  # Clear union_variables so no SAGs get 'Unknown' status in final output
  union_variables <- character(0)
}
...
write.table(QC_non_mock,
            file = file.path(workdir, "05_first_QC", "QC_non_mock-genus.tsv"),
            sep  = "\t",
            row.names = FALSE,
            quote = FALSE)
